...
**Why**: It's the one-shot way to produce the project context doc he writes by hand when kicking off Crafted/Praxis work, and it matches his plan-first, low-friction style.
**Analogy**: A codebase MRI — one scan and you get the full chart instead of poking around for symptoms.
**Suggested next step**: Run it against an existing Crafted/Praxis repo and diff the generated `AGENTS.md` against the hand-written one to find blind spots.
