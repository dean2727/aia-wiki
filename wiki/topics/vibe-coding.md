...
Workflow: start from a strong template; plan in Markdown before coding; keep plans in-repo (`plans/implementation-plan.md`); distill patterns/frustrations into rules (AGENTS.md); separate planning from execution (cheap models draft, strong models review). Recovery: when a section goes off the rails, redo it with proper planning rather than patching messy code. See [[spec-driven-development]] and [[starting-a-project-vibe-coding]] for the structured form, and [[fix-tests-workflow]] for distilling repeated frustrations into a workflow.
