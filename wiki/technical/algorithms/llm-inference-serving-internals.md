...
**Adoption path**: watch
**Why**: All three are serving-layer internals on the *provider* side of the OpenRouter boundary — you don't operate batchers, RL rollout engines, or self-host diffusion checkpoints, so there's nothing to adopt hands-on. The value is diagnostic. (a) and (c) explain why your Praxis agent's latency behaves as it does: each agent step is a *single, small-batch* generation, which is the regime where AR decoding is most memory-bound and where async-batching throughput wins least help you (those gains favor large concurrent batches). That's *why* a long agent loop feels latency-bound rather than compute-bound, and why "fast" model tiers (often speculative/parallel decoding under the hood, à la self-speculation) can cut wall-clock without changing the model. (b) is the one to internalize conceptually: it's the same class of train/inference-parity bug that quietly degrades the hosted models you depend on — useful skepticism when a provider's "same" model behaves differently after an engine update. Self-hosting math would only change the calculus if Praxis ever needed batch-1 latency floors a provider can't hit, or a revision/fill-in-the-middle capability — then a diffusion-mode open checkpoint becomes worth real evaluation.
**Analogy**: Async batching is a **restaurant kitchen** where the chef (GPU) used to stand idle while the expediter (CPU) plated and called the next order — now the expediter preps the next ticket *while* the chef cooks, so the stove is never cold. AR vs diffusion decoding is **typing a sentence letter-by-letter vs. sketching the whole sentence lightly and inking in the words you're already confident about** — same hand, far fewer strokes. And the vLLM RL bug is a **scale miscalibrated by a hair**: weigh every ingredient against it and the recipe drifts, but you'll blame the recipe (the objective) long before you suspect the scale (the backend).
**Suggested next step**: None hands-on. Keep the mental model for diagnosing latency: when a Praxis agent trace feels slow, ask *which* layer — per-step memory-bound decoding (the AR ceiling), accumulated KV cache (see [[deepseek-v4]]), or orchestration overhead — and remember that batch-1 agent calls sit in the worst regime for the throughput tricks above. Treat any provider claim of "same model, now faster" as a serving-layer change, not a capability change.
